				...
				for (int y = num5; y < num6; y++)
				{
					if (Main.tile[x, y] != null)
					{
						...
						if (liquid.HasLiquid())
						{
							...
							vector2.Y += num8 * 2f;
							num7 -= (int) (num8 * 2f);
							if (vector.X + (float) num > vector2.X && vector.X < vector2.X + 16f &&
							    vector.Y + (float) num2 > vector2.Y && vector.Y < vector2.Y + (float) num7)
							{
						...
						else if (Main.tile[x, y].active() && Main.tile[x, y].slope() != 0 && y > 0 &&
						         Main.tile[x, y - 1] != null && Main.tile[x, y - 1].liquid > 0)
						{
							...
							int num9 = 16;
							if (vector.X + (float) num > vector2.X && vector.X < vector2.X + 16f &&
							    vector.Y + (float) num2 > vector2.Y && vector.Y < vector2.Y + (float) num9)
							{
								{
									for (byte b = 3; b < 2 + LiquidRegistry.liquidList.Count; b++)
									{
